...
public class GosControlModuleBaseActivity extends GosBaseActivity {
	 ...
	 * ===========================================================
	 */
	// 数据点"Remaining_Pack"对应的标识名
	protected static final String KEY_REMAINING_PACK = "Remaining_Pack";
	// 数据点"Lid_State"对应的标识名
	protected static final String KEY_LID_STATE = "Lid_State";
	// 数据点"D_Remaining_Pack"对应的标识名
	protected static final String KEY_D_REMAINING_PACK = "D_Remaining_Pack";
	// 数据点"Time_Now"对应的标识名
	protected static final String KEY_TIME_NOW = "Time_Now";
	// 数据点"Dosing_Time"对应的标识名
	protected static final String KEY_DOSING_TIME = "Dosing_Time";
	 ...
	 * ===========================================================
	 */
	// 数据点"Remaining_Pack"对应seekbar滚动条补偿值
	protected static final int REMAINING_PACK_OFFSET = 0;
	// 数据点"Remaining_Pack"对应数据点增量值
	protected static final int REMAINING_PACK_ADDITION = 0;
	// 数据点"Remaining_Pack"对应数据点定义的分辨率
	protected static final int REMAINING_PACK_RATIO = 1;

	// 数据点"Time_Now"对应seekbar滚动条补偿值
	protected static final int TIME_NOW_OFFSET = 0;
	// 数据点"Time_Now"对应数据点增量值
	protected static final int TIME_NOW_ADDITION = 0;
	// 数据点"Time_Now"对应数据点定义的分辨率
	protected static final int TIME_NOW_RATIO = 1;
	 ...
	 * ===========================================================
	 */
	// 数据点"Remaining_Pack"对应的存储数据
	protected static int data_Remaining_Pack;
	// 数据点"Lid_State"对应的存储数据
	protected static int data_Lid_State;
	// 数据点"D_Remaining_Pack"对应的存储数据
	protected static int data_D_Remaining_Pack;
	// 数据点"Time_Now"对应的存储数据
	protected static int data_Time_Now;
	// 数据点"Dosing_Time"对应的存储数据
	protected static byte[] data_Dosing_Time;
	...
	protected void getDataFromReceiveDataMap(ConcurrentHashMap<String, Object> dataMap) {
		...
		if (dataMap.get("data") != null) {
			...
			for (String dataKey : map.keySet()) {
				if (dataKey.equals(KEY_REMAINING_PACK)) {

					data_Remaining_Pack = (Integer) map.get(dataKey);
				}
				if (dataKey.equals(KEY_LID_STATE)) {

					data_Lid_State = (Integer) map.get(dataKey);
				}
				if (dataKey.equals(KEY_D_REMAINING_PACK)) {

					data_D_Remaining_Pack = (Integer) map.get(dataKey);
				}
				if (dataKey.equals(KEY_TIME_NOW)) {

					data_Time_Now = (Integer) map.get(dataKey);
				}
				if (dataKey.equals(KEY_DOSING_TIME)) {

					data_Dosing_Time = (byte[]) map.get(dataKey);
				}
			}
		}
